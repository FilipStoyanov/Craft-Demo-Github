const updateContactFromFreshDeskLib = require("../../src/fetch/index.ts");
...
describe("Update User from Github API", () => {
    test("should return invalid credentials when the token is invalid", async () => {
        const data = await updateContactFromFreshDeskLib.updateContactFromFreshDesk( updatedContact, VALID_SUBDOMAIN, CONTACT_ID, INVALID_KEY );
    ...
    test("should return invalid credentials when the subdomain is invalid", async () => {
        const data = await updateContactFromFreshDeskLib.updateContactFromFreshDesk( updatedContact, INVALID_SUBDOMAIN, CONTACT_ID , VALID_KEY);
        expect(data).toHaveProperty('code', 'invalid_credentials');
    });
    test("should return invalid credentials when the subdomain is invalid and the token is invalid", async () => {
        const data = await updateContactFromFreshDeskLib.updateContactFromFreshDesk( updatedContact, INVALID_SUBDOMAIN, CONTACT_ID ,INVALID_KEY);
        expect(data).toHaveProperty('code', 'invalid_credentials');
    });
    test("should return contact object when the token and the subdomain are valid", async () => {
        const data = await updateContactFromFreshDeskLib.updateContactFromFreshDesk( updatedContact, VALID_SUBDOMAIN, CONTACT_ID ,VALID_KEY);
